...
function StatCard({ icon, labelKey, value, change }: StatCardProps) {
  ...
  return (
    <div className="bg-sacred-surface border border-gold/20 rounded-lg p-6 hover:border-gold/40 transition-all duration-300">
      ...
      <p className="text-gray-400 text-sm mb-1">{t(labelKey)}</p>
...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      labelKey: 'dash.nfc.scans',
      value: 0,
      change: "+0%",
    },
    {
      icon: <Users className="w-6 h-6" />,
      labelKey: 'dash.waitlist',
      value: 0,
      change: "+0%",
    },
  ];
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-bold text-white mb-2">{t("dash.title")}</h1>
      ...
      <div className="bg-sacred-surface border border-gold/20 rounded-lg p-6">
        <h2 className="text-xl font-bold text-gold mb-4">{t("dash.mission")}</h2>
        <div className="mb-2 flex justify-between">
          <span className="text-gray-300">{t("dash.keys.activated")}</span>
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        <button className="bg-sacred-surface border border-gold/20 hover:border-gold/50 rounded-lg p-6 transition-all duration-300 text-left group">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-bold text-white">{t("dash.action.process")}</h3>
          </div>
          <p className="text-sm text-gray-400">{t("dash.action.process.desc")}</p>
        </button>

        <button className="bg-sacred-surface border border-gold/20 hover:border-gold/50 rounded-lg p-6 transition-all duration-300 text-left group">
          <div className="flex items-center gap-3 mb-4">
            <Mail className="w-6 h-6 text-gold group-hover:text-gold/80" />
            <h3 className="font-bold text-white">{t("dash.action.email")}</h3>
          </div>
          <p className="text-sm text-gray-400">{t("dash.action.email.desc")}</p>
        </button>

        <button className="bg-sacred-surface border border-gold/20 hover:border-gold/50 rounded-lg p-6 transition-all duration-300 text-left group">
          <div className="flex items-center gap-3 mb-4">
            <Plus className="w-6 h-6 text-gold group-hover:text-gold/80" />
            <h3 className="font-bold text-white">{t("dash.action.contact")}</h3>
          </div>
          <p className="text-sm text-gray-400">{t("dash.action.contact.desc")}</p>
        </button>
      </div>

      {/* Chart Placeholder */}
      <div className="bg-sacred-surface border border-gold/20 rounded-lg p-8">
        <h2 className="text-xl font-bold text-gold mb-6">{t("dash.activity")}</h2>
